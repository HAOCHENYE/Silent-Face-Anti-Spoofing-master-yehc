...
class LRFinder(object):
    ...
    def _train_batch(self, train_iter, accumulation_steps, non_blocking_transfer=True):
        ...
        for i in range(accumulation_steps):
            # import pdb
            # pdb.set_trace()
            inputs, FT_image, labels = next(train_iter)
            inputs, labels = self._move_to_device(
                inputs, labels, non_blocking=non_blocking_transfer
            )
            FT_image, labels = self._move_to_device(
                FT_image, labels, non_blocking=non_blocking_transfer
            )
            # import pdb
            # pdb.set_trace()
            # Forward pass

            outputs,outputs1,_,_ = self.model(inputs)
            # outputs1 = self.model(FT_image)
            loss = self.criterion(outputs, labels)
            loss1 = self.criterion1(outputs1, FT_image)
    ...
    def plot(
        ...
        if fig is not None:
            plt.show()
            # plt.savefig("/ubuntu/home/Silent-Face-Anti-Spoofing-master/lose.jpg",ax)
