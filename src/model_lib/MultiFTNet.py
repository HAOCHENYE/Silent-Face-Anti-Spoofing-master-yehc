...
class MultiFTNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.model.bn(x1)
        feature = self.model.drop(x1)
        cls = self.model.prob(feature)

        if self.training:
            ft = self.FTGenerator(x)
            return cls, ft, feature, self.model.fc
        else:
            return cls
            # ft = self.FTGenerator(x)
            # return cls, ft, feature, self.model.fc
    ...
    def load(self, file):
        ...
        for name, parameters in checkpoint.items():
            new_name =str(name).replace('module.','')
